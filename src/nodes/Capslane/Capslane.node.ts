...
export class Capslane implements INodeType {
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      { displayName: 'Mode', name: 'mode', type: 'options', default: 'auto', options: [{ name: 'Auto', value: 'auto', description: 'Use native captions and generate only when unavailable' }, { name: 'Native', value: 'native', description: 'Never start audio transcription' }, { name: 'Generate', value: 'generate', description: 'Use audio transcription directly' }] },
      { displayName: 'Language', name: 'lang', type: 'string', default: '', placeholder: 'en', description: 'Optional preferred ISO language code' },
      { displayName: 'Plain Text', name: 'text', type: 'boolean', default: false, description: 'Whether to return one text string instead of timestamped segments' },
      { displayName: 'Wait for Generated Transcript', name: 'wait', type: 'boolean', default: true, description: 'Whether to poll an accepted generation job until completion' },
      { displayName: 'Polling Interval', name: 'interval', type: 'number', default: 2, typeOptions: { minValue: 1, maxValue: 30 }, displayOptions: { show: { wait: [true] } }, description: 'Seconds between job status checks' },
    ],
  }
