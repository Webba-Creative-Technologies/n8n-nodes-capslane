...
export class Capslane implements INodeType {
  description: INodeTypeDescription = {
    ...
    name: 'capslane',
    icon: { light: 'file:capslane.svg', dark: 'file:capslane.dark.svg' },
    group: ['transform'],
    version: 1,
    ...
    properties: [
      ...
      { displayName: 'Wait for Generated Transcript', name: 'wait', type: 'boolean', default: true, description: 'Whether to wait for an accepted transcript job before continuing' },
      ...
      try {
        ...
        if (wait && typeof result.jobId === 'string') {
          ...
          while (Date.now() < deadline && !('content' in result)) {
            ...
            if (result.status === 'failed' || result.status === 'cancelled') throw new NodeOperationError(this.getNode(), new Error(`Transcript job ${String(result.status)}`), { itemIndex: index })
          }
          if (!('content' in result)) throw new NodeOperationError(this.getNode(), new Error('Transcript job deadline exceeded'), { itemIndex: index })
        }
